...
/* ---------------------------- DETECCIÓN DE BANCO / ESTILO VISUAL DE TARJETA ---------------------------- */
function normalizeText(s) {
  return String(s || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
}

const BANK_PALETTES = [
  { id: "bbva", label: "BBVA", keys: ["bbva"], gradient: "linear-gradient(135deg, #1B5FA8 0%, #0A2547 100%)", fg: "silver" },
  { id: "banorte", label: "Banorte", keys: ["banorte"], gradient: "linear-gradient(135deg, #C1272D 0%, #4E0E10 100%)", fg: "silver" },
  { id: "amex", label: "American Express", keys: ["american express", "amex"], gradient: "linear-gradient(135deg, #086F52 0%, #0A1F1B 100%)", fg: "gold" },
  { id: "plata", label: "Plata Card", keys: ["plata card", "plata"], gradient: "linear-gradient(135deg, #3A3B40 0%, #0B0B0D 100%)", fg: "silver" },
  { id: "citibanamex", label: "Citibanamex", keys: ["citibanamex", "banamex"], gradient: "linear-gradient(135deg, #0056A4 0%, #7A0C1E 100%)", fg: "silver" },
  { id: "santander", label: "Santander", keys: ["santander"], gradient: "linear-gradient(135deg, #EA1D25 0%, #5C0000 100%)", fg: "silver" },
  { id: "hsbc", label: "HSBC", keys: ["hsbc"], gradient: "linear-gradient(135deg, #DB0011 0%, #191919 100%)", fg: "silver" },
  { id: "nu", label: "Nu", keys: ["nubank", "nu"], gradient: "linear-gradient(135deg, #9526D6 0%, #390A5C 100%)", fg: "silver" },
  { id: "klar", label: "Klar", keys: ["klar"], gradient: "linear-gradient(135deg, #232323 0%, #000000 100%)", fg: "gold" },
  { id: "scotiabank", label: "Scotiabank", keys: ["scotiabank", "scotia"], gradient: "linear-gradient(135deg, #EC111A 0%, #6B0000 100%)", fg: "silver" },
  { id: "banregio", label: "Banregio", keys: ["banregio"], gradient: "linear-gradient(135deg, #F2622E 0%, #7A2408 100%)", fg: "gold" },
  { id: "banbajio", label: "BanBajío", keys: ["banbajio", "bajio"], gradient: "linear-gradient(135deg, #00704A 0%, #00301F 100%)", fg: "gold" },
  { id: "mercadolibre", label: "Mercado Pago", keys: ["mercadolibre", "mercado libre", "mercado pago", "mercadopago"], gradient: "linear-gradient(135deg, #2D3277 0%, #12142E 100%)", fg: "gold" },
];
const DEFAULT_PALETTE = { id: "default", label: "", gradient: "linear-gradient(135deg, #232A33 0%, #10141A 100%)", fg: "gold" };

function detectBankPalette(name) {
  const n = normalizeText(name);
  for (const bank of BANK_PALETTES) {
    for (const key of bank.keys) {
      const k = normalizeText(key);
      const re = new RegExp(`(^|[^a-z0-9])${k.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}([^a-z0-9]|$)`);
      if (re.test(n)) return bank;
    }
  }
...
const STYLE_PRESETS = {
  oro: { label: "Oro", gradient: "linear-gradient(135deg, #C9A24B 0%, #5B4720 100%)", fg: "gold" },
...
function getCardVisual(card) {
  ...
  if (override === "personalizado" && card.customColor) {
    return { gradient: `linear-gradient(135deg, ${card.customColor} 0%, #0B0B0D 100%)`, fg: "silver", bankLabel: "" };
  }
  if (override !== "auto" && STYLE_PRESETS[override]) {
    const p = STYLE_PRESETS[override];
    return { gradient: p.gradient, fg: p.fg, bankLabel: "" };
  }
  const bank = detectBankPalette(card.name);
  return { gradient: bank.gradient, fg: bank.fg, bankLabel: bank.label || "" };
}

function cardVisualHTML(card, disponible) {
  const v = getCardVisual(card);
  return `
  <div class="credit-card-visual fg-${v.fg}" style="background:${v.gradient};">
    <div class="ccv-shine"></div>
    <div class="ccv-top">
      <span class="ccv-bank">${v.bankLabel ? esc(v.bankLabel) : "Tarjeta"}</span>
      <span class="ccv-chip"></span>
    </div>
    <div class="ccv-bottom">
      <div>
        <div class="ccv-label">Titular / Alias</div>
        <div class="ccv-name">${esc(card.name)}</div>
      </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
          ...
          <div style="grid-column:1/-1;">
            ...
            <select id="card-style-${card.id}" data-card-style="${card.id}">
              <option value="auto" ${(!card.styleOverride || card.styleOverride === "auto") ? "selected" : ""}>Automático (detectar banco por nombre)</option>
            ...
            <div id="card-customcolor-wrap-${card.id}" style="margin-top:8px;display:${card.styleOverride === "personalizado" ? "flex" : "none"};align-items:center;gap:10px;">
              <input type="color" id="card-customcolor-${card.id}" value="${card.customColor || "#C9A24B"}" style="width:44px;height:34px;padding:2px;border-radius:6px;border:1px solid var(--border);background:var(--panel2);cursor:pointer;">
